...
export default function PollComposer({ onChange, onRemove }: PollComposerProps) {
  ...
  return (
    <motion.div
      ...
      className="rounded-2xl border border-border/35 bg-background px-4 py-4 shadow-sm sm:px-5"
    >
      {/* ── Header ──────────────────────────────────────────── */}
          ...
          {options.map((opt, i) => (
            ...
            >
              ...
              <input
                ...
                className="h-9 flex-1 rounded-xl border border-border/35 bg-card px-3.5 text-sm text-card-foreground outline-none placeholder:text-muted-foreground/60 focus:border-primary/50 focus:ring-1 focus:ring-primary/20"
              />
